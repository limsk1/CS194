class LoadCsUnspecialized < ActiveRecord::Migration
  def up
    unspeicalized = Track.new(name: 'CS-Unspecialized')
    unspeicalized.save!

    math = Category.new(show_name: 'Math', min_units: 26, min_courses: 6)
    math.track = unspeicalized
    math.save!
    ...
    track.track = unspeicalized
    track.save!

    cs_154 = Requirement.new(course_id: Course.find_by(course_name: 'CS154').id, num_courses: 1, priority: 4, repeatable: false)
    cs_154.category = track
    cs_154.save!

    cs_154_ful = Fulfillment.new
    cs_154_ful.requirement = cs_154
    cs_154_ful.course = Course.find_by(course_name: 'CS154')
    cs_154_ful.save!
    ...
    track_c_list = track_b_list + ['CS144', 'CS155', 'CS242', 'CS244', 'EE180']
    ...
    track_d = Requirement.new(num_courses: 1, criteria: 'Track Requirement D', priority: 2, repeatable: false)
    track_d.category = track
    track_d.save!

    track_d_list = ['CS221', 'CS223A', 'CS228', 'CS229', 'CS231A']

    track_d_list.each do |elec|
        track_d_ful = Fulfillment.new
        track_d_ful.requirement = track_d
        if Course.find_by(course_name: elec) == nil then 
            Course.create(course_name: elec)
        end
        track_d_ful.course = Course.find_by(course_name: elec)
        track_d_ful.save!    
    end 

    track_e = Requirement.new(num_courses: 1, criteria: 'Track Requirement E', priority: 2, repeatable: false)
    track_e.category = track
    track_e.save!

    track_e_list = ['CS145', 'CS147', 'CS148', 'CS248', 'CS262']

    track_e_list.each do |elec|
        track_e_ful = Fulfillment.new
        track_e_ful.requirement = track_e
        if Course.find_by(course_name: elec) == nil then 
            Course.create(course_name: elec)
        end
        track_e_ful.course = Course.find_by(course_name: elec)
        track_e_ful.save!    
    end 

    track_elec = Requirement.new(num_courses: 2, criteria: 'Track Electives', priority: 1, repeatable: false)
